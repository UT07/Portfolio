...
const PressKit = () => {
  const { pressKit, hero } = djData;
  const quickFacts = [
    'Based in Dublin',
    'Tempo 155–180 BPM',
    `Genres: ${hero.genres}`,
    'Core member: Redline collective'
  ];
      ...
      <div className="max-w-7xl mx-auto px-6 md:px-12 relative z-10">
        ...
        <div className="mb-16">
          <motion.div
            ...
            viewport={{ once: true }}
            transition={{ duration: 0.6 }}
            className="border border-white/10 p-6 bg-gradient-to-br from-black/60 via-neutral-900/50 to-black/60"
            style={{ boxShadow: '0 0 20px rgba(217,70,239,0.15)' }}
          >
            <h3 className="text-lg font-bold text-white uppercase tracking-widest font-unbounded mb-4">
              Quick Facts
            </h3>
            <div className="flex flex-wrap gap-3">
              {quickFacts.map((fact, index) => (
                <span
                  key={index}
                  className="px-4 py-2 text-xs font-space-mono uppercase tracking-wider border border-cyan-500/40 text-cyan-300 bg-black/40"
        ...
        {pressKit.technical_rider && (
          <div className="mb-16">
            <motion.div
              ...
              viewport={{ once: true }}
              transition={{ duration: 0.6 }}
              className="bg-neutral-900/70 border border-cyan-500/30 p-8"
              style={{
                boxShadow: '0 0 20px rgba(6,182,212,0.2)'
              }}
            >
              <h3 className="text-xl font-bold text-cyan-400 mb-3 uppercase tracking-widest font-unbounded">
                {pressKit.technical_rider.title}
              </h3>
              <p className="text-sm text-neutral-400 mb-6 font-space-mono">
                {pressKit.technical_rider.notes}
              </p>
              <ul className="space-y-3">
                {pressKit.technical_rider.items.map((item, index) => (
                  <li
                    key={index}
                    className="text-sm text-neutral-300 font-space-mono flex items-start gap-3"
                  >
