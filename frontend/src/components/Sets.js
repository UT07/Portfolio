                ...
                <div className="flex flex-col sm:flex-row sm:items-center gap-4 sm:gap-6 mb-8">
                  ...
                  >
                    {(() => {
                      ...
                      return (
                        <div
                          className="w-16 h-16 sm:w-20 sm:h-20 rounded-2xl border border-red-500/40 bg-neutral-950/60 flex items-center justify-center text-red-400"
                          style={{
                            boxShadow: `0 0 30px rgba(${accentColor === 'red' ? '255,26,64' : '255,94,112'},0.4)`
                          }}
                        >
                          {Icon ? <Icon className="w-8 h-8 sm:w-10 sm:h-10" /> : <Play className="w-7 h-7" />}
                        </div>
                      );
                    })()}
                  </motion.div>
                  <div className="min-w-0">
                    <h3 className="text-2xl sm:text-3xl font-bold text-white uppercase tracking-wide sm:tracking-wider font-unbounded mb-2 break-words">
                      {platform.name}
                    </h3>
                    <motion.button
                      onClick={() => openExternal(platform.url)}
                      className={`w-full sm:w-auto justify-center flex items-center gap-2 px-4 py-2 border ${
                        accentColor === 'red'
